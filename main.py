...
async def add_user_to_database(username:str, email: str, password: str):
    db[username] = {
        "username": username,
        "email": email,
...
@app.post("/register")
async def login_for_access_toke(form_data: RegisterRequestForm):
    await add_user_to_database(form_data.username, form_data.email, form_data.password)
    print(db)
